...
static int open_win32(struct audio_output_struct* ao)
{
	...
	state = (queue_state*)calloc(1,sizeof(struct queue_state));
	if(!state) 
        return -1;

	ao->userptr = state;
	for(i = 0; i < NUM_BUFFERS; i++)
	if(!(state->buffer_headers[i].lpData = (CHAR*)malloc(BUFFER_SIZE)))
	    return -1;
...
static void flush_win32(struct audio_output_struct *ao)
{
	...
	for(i = 0; i < NUM_BUFFERS; i++)
	{
		if(!state->buffer_headers[i].dwFlags & WHDR_DONE)
			WaitForSingleObject(state->play_done_event,0);//INFINITE);
...
int32_t init_winmm(audio_output_t* ao)
{
	if(!ao) 
        return -1;
    
    ao->id = AUDIO_DRIVER_WINMM;
	ao->open = open_win32;
...
int deinit_winmm(audio_output_t* ao)
{
	return 0;
} 
