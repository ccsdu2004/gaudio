...
int32_t gphasereffectcreator::is_support(int32_t dec,int32_t mask)
{   
    if(dec == AUDIO_EFFECT_PHASER)
       return TRUE;
    return FALSE;            
} 
 
geffect* gphasereffectcreator::create_by_id(int32_t dec,int32_t mask)
{
    if(dec == AUDIO_EFFECT_PHASER)
...
void gphasereffectcreator::destroy()
{
    delete this; 
}
...
gaddons* GAPIENTRY ginstance(void* ctx)
{
    ...
    addon.addon = new gphasereffectcreator();
    return &addon;
}
