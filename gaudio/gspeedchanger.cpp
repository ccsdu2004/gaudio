...
geffect* gspeedchanger_effect_create();
...
{
    if(dec == AUDIO_EFFECT_SPEED_CHANGER)
       return gspeedchanger_effect_create();
    return NULL;
}
...
void speedchanger_effect_init(geffectimpl* changer)
{
    changer->ratio = AUDIO_EFFECT_SPEED_CHANGER_RATIO_DEFAULT;
    changer->update = FALSE;
}

static const char* speedchanger_effect_maker(geffect*)
{
    return "http://www.geek-audio.org/";
}

void speedchanger_effect_destroy(geffect* effect)
{
...
void GAPIENTRY speedchanger_effect_process(geffect* effect,gdatabuffer<float>& buffer)
{
    geffectimpl* changer = effect->impl;
    if(changer->update)
        changer->update = FALSE;
    ...
    for(int i=0;i<ret1;i++)
    {
        ...
        //v = gfilter2p(&changer->filter[0],0,v);
    ...
    for(int i=0;i<ret2;i++)
    {
        ...
        //v = gfilter2p(&changer->filter[1],0,v);
...
void GAPIENTRY speedchanger_effect_set_float(geffect* effect,int32_t flag,float value)
{
...
void GAPIENTRY speedchanger_effect_get_float(geffect* effect,int32_t flag,float* value)
{
    if(flag != AUDIO_EFFECT_SPEED_CHANGER_RATIO)
        return;
    geffectimpl* changer = effect->impl;
...
void GAPIENTRY speedchanger_effect_set_int32(geffect* effect,int32_t flag,int32_t value)
{
...
void GAPIENTRY speedchanger_effect_get_int32(geffect* effect,int32_t flag,int32_t* value)
{
    if(flag != AUDIO_EFFECT_SPEED_CHANGER_RATIO + 1)
        return;
    flag --;
...
geffect* gspeedchanger_effect_create()
{
    ...
    effect->binded = FALSE;
    effect->fftcb = 0;
	effect->process = speedchanger_effect_process;
    effect->order = -1;
    effect->source = 0;
    effect->destroy = speedchanger_effect_destroy;
    effect->setfloat = speedchanger_effect_set_float;
    effect->getfloat = speedchanger_effect_get_float;
    effect->setint = 0;
    effect->getint = 0;
    effect->state = TRUE;

    geffectimpl* changer = new geffectimpl();
    speedchanger_effect_init(changer);
    effect->impl = changer;
    return effect;
}
