...
static sint32 getvl(streamBuffer *stream)
{
  sint32 l=0;
  uint8 c;
  for (;;)
    {
      stream_read(stream, &c, 1, 1);
      l += (c & 0x7f);
      if (!(c & 0x80)) return l;
      l<<=7;
    }
...
static int read_meta_data(streamBuffer *stream, sint32 len, uint8 type, MidSong *song)
{
  ...
  if (len != (sint32) stream_read(stream, s, 1, len))
    {
      free(s);
      return -1;
    }
...
static MidEventList *read_midi_event(streamBuffer *stream, MidSong *song)
{
  ...
  for (;;)
    {
      ...
      if (stream_read(stream, &me, 1, 1) != 1)
	{
	...
	{
	  len=getvl(stream);
	  stream_seek(stream,len,SEEK_CUR);
	}
      else if(me==0xFF) /* Meta event */
	{
	  stream_read(stream, &type, 1, 1);
	  len=getvl(stream);
	  if (type>0 && type<16)
	    {
		...
		return MAGIC_EOT;
	      ...
	      case 0x51: /* Tempo */
		stream_read(stream, &a, 1, 1);
		stream_read(stream, &b, 1, 1);
		stream_read(stream, &c, 1, 1);
...
static int read_track(streamBuffer *stream, MidSong *song, int append)
{
  MidEventList *meep;
  ...
  if (stream_read(stream, tmp, 1, 4) != 4 || stream_read(stream, &len, 4, 1) != 1)
    {
...
MidEvent *read_midi_file(streamBuffer *stream, MidSong *song, sint32 *count, sint32 *sp)
{
  ...
  stream_read(stream, &format, 2, 1);
  stream_read(stream, &tracks, 2, 1);
  stream_read(stream, &divisions_tmp, 2, 1);
  ...
  if (len > 6)
    {
      ...
      stream_seek(stream,len-6,SEEK_CUR); /* skip the excess */
    }
