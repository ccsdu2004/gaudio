...
extern RIFF_Chunk* LoadRIFF(streamBuffer *stream);
...
RIFF_Chunk *LoadRIFF(streamBuffer *stream)
{
    RIFF_Chunk *chunk;
    uint8 *subchunkData;
    uint32 subchunkDataLen;
    ...
    stream_read(stream, &tmpUint32, sizeof(uint32), 1);
    ...
    if (stream_read(stream, chunk->data, chunk->length, 1) != 1 ) {
	DEBUG_MSG("IO error\n");
        FreeRIFF(chunk);
        return NULL;
    }
...
extern MidDLSPatches* mid_dlspatches_load(streamBuffer *stream);
...
MidDLSPatches *mid_dlspatches_load(streamBuffer *stream)
{
    RIFF_Chunk *chunk;
