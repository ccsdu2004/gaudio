...
int32_t stdio_stream_read(void* ptr,int32_t size,int32_t nmemb,void* ctx)
{
    return fread(ptr,size,nmemb,((stdIOContext*)ctx)->fp);
}

int32_t stdio_stream_seek(void* ctx,int32_t offset,int32_t flag)
{
    return fseek(((stdIOContext*)ctx)->fp,offset,flag);
}

int32_t stdio_stream_tell(void* ctx)
{
    return ftell(((stdIOContext*)ctx)->fp);
}

int32_t stdio_stream_close(void* ptr)
{
...
int32_t mem_stream_read(void* ptr,int32_t size,int32_t nmemb,void* ctx)
{
...
int32_t mem_stream_seek(void* ctx,int32_t offset,int32_t flag)
{
    streamBuffer* stream = (streamBuffer*)ctx;
    memContext* c;
    c = (memContext*)stream;
...
int32_t mem_stream_tell(void *ptr)
{
    memContext* ctx = (memContext*)ptr;
    return ctx->current - ctx->base;
}

int32_t mem_stream_close(void *ptr)
{
    memContext* ctx = (memContext*)ptr;
    if(ctx->close)
        free(ctx->base);
...
streamBuffer* stream_open_fp(FILE* fp,const char* file)
{
    ...
    stream = malloc(sizeof(streamBuffer));
    if(stream == NULL)
        return NULL;

    ctx = malloc(sizeof(stdIOContext));
    if(ctx == NULL)
    {
        free(stream);
        return NULL;
...
{
    streamBuffer* stream = malloc(sizeof(streamBuffer));
    if(stream == NULL)
       return NULL;
